...
import { getProfile, type Profile, type ExtraLink } from "@/lib/content";

const DEFAULT_ROLES = ["AI Engineer", "Full-Stack Dev", "RAG Architect", "Backend Engineer"];
...
export default function HeroSection() {
  ...
  const roles = profile?.roles ?? DEFAULT_ROLES;
          ...
          <div>
            ...
            {(profile?.open_to_work ?? true) && (
              <div className="animate-fade-up" style={{
                ...
                background: "rgba(0,255,170,0.08)", marginBottom: 32,
              }}>
            ...
            <div className="animate-fade-up delay-100" style={{ display: "flex", alignItems: "flex-end", gap: 20, marginBottom: 20 }}>
              ...
              <div style={{
                ...
                fontSize: 28, color: "rgba(157,143,240,0.8)",
              }}>
                {!profile?.avatar_url && (profile?.name?.[0] ?? "M")}
              </div>

              <h1 className="font-display" style={{ lineHeight: 0.92 }}>
                <span style={{ display: "block", fontSize: "clamp(44px, 7vw, 88px)", fontWeight: 900, color: "white" }}>
                  {profile?.name?.split(" ")[0] ?? "Milan"}
                </span>
                <span style={{ display: "block", fontSize: "clamp(44px, 7vw, 88px)", fontWeight: 900, background: "linear-gradient(135deg, #9d8ff0, #00d4ff)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            ...
            {/* Typewriter role */}
            <div className="animate-fade-up delay-200" style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              <div style={{ width: 32, height: 2, background: "linear-gradient(90deg, #7c6fcd, #00d4ff)", flexShrink: 0 }} />
              <p className="font-display" style={{ fontSize: "clamp(15px, 2.5vw, 20px)", fontWeight: 600, color: "#cbd5e1" }}>
                {displayed}
                <span style={{ display: "inline-block", width: 2, height: "1em", background: "#00d4ff", marginLeft: 2, verticalAlign: "middle", animation: "blink-cursor 1s step-end infinite" }} />
              </p>
            </div>

            {/* Bio */}
            <p className="animate-fade-up delay-300" style={{ color: "#94a3b8", fontSize: "clamp(14px, 1.8vw, 16px)", lineHeight: 1.75, maxWidth: 500, marginBottom: 32 }}>
              {profile?.bio ?? "I build production-grade systems where modern web engineering meets real AI — RAG pipelines, vector search, and full-stack applications built to ship."}
            </p>

            {/* Social links row */}
            <div className="animate-fade-up delay-300" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
              {/* GitHub */}
              ...
              <a href={li} target="_blank" rel="noopener noreferrer" style={{
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "#0a66c2"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(10,102,194,0.4)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "#94a3b8"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.1)"; }}
                title="LinkedIn"
              >
              ...
              {/* Email */}
              <a href={`mailto:${profile?.email ?? "milan@email.com"}`} style={{
                width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center",
                background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)",
                color: "#94a3b8", textDecoration: "none", transition: "all 0.2s",
              }}
            ...
            <div className="cta-row animate-fade-up delay-400">
              <a href="#projects" style={{
                padding: "13px 26px", borderRadius: 12, fontWeight: 600, fontSize: 14,
                background: "linear-gradient(135deg, #7c6fcd, #9d8ff0)", color: "white",
                textDecoration: "none", boxShadow: "0 0 32px rgba(124,111,205,0.4)",
              ...
              <a href={profile?.resume_url ?? "/resume.pdf"} style={{
                padding: "13px 26px", borderRadius: 12, fontWeight: 600, fontSize: 14,
